...
void checkTheWordXXX(char* word,int checkDoc)
{
	int							correct;
	AspellWordList*				suggestions;
	AspellStringEnumeration*	elements;
...
void doChangeWordXXX(GtkWidget* widget,gpointer data)
{
//	pageStruct*	page=getPageStructPtr(-1);
	GtkTextIter	start;
	GtkTextIter	end;
...
//	GtkTextIter				end;
	...
	gtk_text_buffer_get_end_iter((GtkTextBuffer*)bufferBox,&end);

	tline=gtk_text_buffer_get_text((GtkTextBuffer*)bufferBox,&start,&end,false);


	fd=fopen("/tmp/aspellcheck","w");
	if (fd!=NULL)
		fprintf(fd,"%s",tline);
	fclose(fd);

	/* Open the file */
	doc=fopen("/tmp/aspellcheck","r");
	if (doc<=0)
		{
	...
	gtk_text_buffer_get_start_iter((GtkTextBuffer*)bufferBox,&start);
	gtk_text_buffer_insert((GtkTextBuffer*)bufferBox,&start,buffer,filelen);
...
void doSpellCheckDocXXX(GtkWidget* widget,gpointer data)
{
//	pageStruct*				page=getPageStructPtr(-1);
	gchar*					buffer;
	long					filelen;
